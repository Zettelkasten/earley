package com.tomerfiliba.earley;
...
public class Earley {
	...
	/*
	 * A CFG rule. Since CFG rules can be self-referential, more productions may
	 * be added to them after construction. For example:
	 * 
	 * Grammar: SYM -> a OP -> + | - EXPR -> SYM | EXPR OP EXPR
	 * 
	 * In Java: Rule SYM = new Rule("SYM", new Production("a")); Rule OP = new
	 * Rule("OP", new Production("+"), new Production("-")); Rule EXPR = new
	 * Rule("EXPR", new Production(SYM)); EXPR.add(new Production(EXPR, OP,
	 * EXPR)); // needs to reference EXPR
	 */
	...
	/*
	 * Represents a state in the Earley parsing table. A state has a its rule's
	 * name, the rule's production, dot-location, and starting- and
	 * ending-column in the parsing table.
	 */
	...
	protected static class TableColumn implements Iterable<TableState>
	{
		...
		/*
		 * only insert a state if it is not already contained in the list of
		 * states. return the inserted state, or the pre-existing one.
		 */
		...
		/*
		 * since we may modify the list as we traverse it, the built-in list
		 * iterator is not suitable. this iterator wouldn't mind the list being
		 * changed.
		 */
	 ...
	 * Usage:
	 * 
	 * Parser p = new Parser(StartRule, "my space-delimited statement"); for
	 * (Node tree : p.getTrees()) { tree.print(System.out); }
	 */
	public static class Parser
	{
		...
		/*
		 * constructor: takes a start rule and a statement (made of
		 * space-separated words). it initializes the table and invokes earley's
		 * algorithm
		 */
		...
		/*
		 * the Earley algorithm's core: add gamma rule, fill up table, and check
		 * if the gamma rule spans from the first column to the last one. return
		 * the final gamma state, or null, if the parse failed.
		 */
		protected TableState parse(Rule startRule)
		{
			...
			for (int i = 0; i < columns.length; i++) {
				...
				// DEBUG -- uncomment to print the table during parsing, column
				// after column
		...
		/*
		 * Earley predict. returns true if the table has been changed, false
		 * otherwise
		 */
		...
		/*
		 * Earley complete. returns true if the table has been changed, false
		 * otherwise
		 */
		...
		/*
		 * call predict() and complete() for as long as the table keeps changing
		 * (may only happen if we've got epsilon transitions)
		 */
		...
		/*
		 * return all parse trees (forest). the forest is simply a list of root
		 * nodes, each representing a possible parse tree. a node is contains a
		 * value and the node's children, and supports pretty-printing
		 */
		...
		/*
		 * this is a bit "magical" -- i wrote the code that extracts a single
		 * parse tree, and with some help from a colleague (non-student) we
		 * managed to make it return all parse trees.
		 * 
		 * how it works: suppose we're trying to match [X -> Y Z W]. we go from
		 * finish-to-start, e.g., first we'll try to match W in X.endCol. let
		 * this matching state be M1. next we'll try to match Z in M1.startCol.
		 * let this matching state be M2. and finally, we'll try to match Y in
		 * M2.startCol, which must also start at X.startCol. let this matching
		 * state be M3.
		 * 
				...
				TableState state, int ruleIndex, TableColumn endCol)
		{
			...
			if (ruleIndex < 0) {
				// this is the base-case for the recursion (we matched the
				// entire rule)
			...
			for (TableState st : endCol) {
				if (st == state) {
					// this prevents an endless recursion: since the states are
					// filled in order of
					// completion, we know that X cannot depend on state Y that
					// comes after it X
					// in chronological order
					break;
				}
				if (!st.isCompleted() || !st.name.equals(rule.name)) {
					// this state is out of the question -- either not completed
					// or does not match
					// the name
					continue;
				}
				if (startCol != null && st.startCol != startCol) {
					// if startCol isn't null, this state must span from
					// startCol to endCol
					continue;
				}
				// okay, so `st` matches -- now we need to create a tree for
				// every possible
				// sub-match
	...
	public static void main(String[] args) throws Exception
	{
		...
		// note that this yields the catalan numbers sequence (as expected) --
		// the number of ways
